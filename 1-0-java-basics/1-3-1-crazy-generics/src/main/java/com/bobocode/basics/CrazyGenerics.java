...
public class CrazyGenerics {
     ...
     */
    interface ComparableCollection<E extends Comparable<E>> extends Collection<E>, Comparable<E> { // todo: refactor it to make generic and provide a default impl of compareTo
    ...
    static class CollectionUtil<T> {
         ...
         */
        public static<T> boolean hasDuplicates(List<T> entityList, T targetEntity) {
            return entityList.stream().filter(e -> e.equals(targetEntity)).count() > 1;
        }
        ...
        public Optional<T> findMax(Iterable<T> elements, Comparator<T> comparator) {
        ...
        public<T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) throws NoSuchElementException {
            if (entities.isEmpty()) {
                throw new NoSuchElementException();
            }
            return entities.stream().sorted(CREATED_ON_COMPARATOR).toList().get(0);
        }
